...
router.get("/:username", auth, (req, res) => {
  const userQuery = User.findOne({ username: req.params.username })
    ...
    .populate("friendRequestsSent", "firstName lastName")
    .populate("friendRequestsReceived", "firstName lastName");
...
router.get("/:username/friendsnippets", auth, async (req, res) => {
  ...
  const snippetQuery = await Snippet.find({
    isPrivate: false,
    owner: [...user.friends],
  }).populate("owner", "firstName lastName");
  if (snippetQuery.length === 0)
    res.status(400).json({ msg: "no snippets found" });
...
router.post("/:username/:folder_id/addsnippet", auth, async (req, res) => {
  try {
    ...
    const foundFolder = await Folder.findByIdAndUpdate(
      newSnippet.parentFolder,
      {
...
router.put("/:username/addfriend/:friend_username", auth, async (req, res) => {
  try {
    ...
    const friendToAdd = await User.findOne({
      username: req.params.friend_username,
    }).select("-password");
    ...
    res
      .status(200)
      .json({
        msg: `Friend Request Sent to ${friendToAdd.firstName} ${friendToAdd.lastName}`,
      });
  } catch (err) {
...
router.put("/:username/approvefriend/:friend_id", auth, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        msg: `Approved: ${updatedFriend.firstName} ${updatedFriend.lastName}`,
      });
  } catch (err) {
    res.status(400).json({ msg: err.message });
  }
...
router.put("/:username/denyfriend/:friend_id", auth, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        msg: `Denied: ${updatedFriend.firstName} ${updatedFriend.lastName}`,
      });
  } catch (err) {
    res.status(400).json({ msg: err.message });
  }
...
// Delete Snippet
router.delete(
  "/:username/snippets/:snippet_id/delete",
  auth,
  async (req, res) => {
    try {
      const deletedSnippet = await Snippet.findByIdAndDelete(
        req.params.snippet_id
      );
      const foundFolder = await Folder.findByIdAndUpdate(
        deletedSnippet.parentFolder,
        {
...
// Delete Folder
router.delete(
  "/:username/folders/:folder_id/delete",
  auth,
  async (req, res) => {
    try {
